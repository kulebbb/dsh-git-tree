...
/** Write a JSON response, never letting a failed write escape as an unhandled rejection. */
function trySend(ctx, res, status, body) {
  try {
    sendJson(res, status, body);
  } catch (error) {
    ctx.logger.warn("[dsh-git-tree] failed to write response:", error);
  }
}
...
export function apply(ctx, config, deps = {}) {
  ...
  ctx.effect(() => {
    ...
    void (async () => {
      try {
        ...
        ctx.logger.debug?.("[dsh-git-tree] update check failed:", error);
      }
    })();
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      if (req.method !== "GET") {
        trySend(ctx, res, 405, { ok: false, error: { code: "method-not-allowed", message: "use GET" } });
        return;
      }
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      if (req.method !== "POST") {
        trySend(ctx, res, 405, { ok: false, error: { code: "method-not-allowed", message: "use POST" } });
        return;
      }
      if (state.updating) {
        trySend(ctx, res, 409, { ok: false, error: { code: "already-running", message: "an update is already in progress" } });
        return;
      }
      if (state.dev) {
        trySend(ctx, res, 409, { ok: false, error: { code: "dev-install", message: "plugin installed via a link/file spec; refusing to auto-update" } });
        return;
      }
      const dir = state.profile?.dir;
      if (!dir) {
        trySend(ctx, res, 409, { ok: false, error: { code: "profile-not-found", message: state.profile?.reason ?? "cannot locate the profile directory" } });
        return;
      }
      state.updating = true;
      try {
        ...
        trySend(ctx, res, result.ok ? 200 : 500, {
          ok: result.ok,
