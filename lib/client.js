window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function layoutGraph(commits) {
      ...
      for (let i = 0; i < commits.length; i++) {
        ...
        for (const p of commit.parents) {
          if (colOf.has(p)) { edges.push({ from: hash, to: p }); continue; }
    ...
    function colorOf(refs) {
      ...
      return name ? `hsl(${hueOf(name)} 62% 55%)` : "var(--dsw-alias-label-secondary, #8b949e)";
    }
    //#endregion
    ...
    function GitTreePanel({ wide, t, useSessions, useWorkspaces, fetchGraph: loadGraph }) {
      ...
      const errorText = useMemo(() => {
        ...
        switch (error) {
          ...
          case "internal": return t("panel.internal", { message: "" });
          default: return t("panel.gitError", { message: error });
        }
      }, [error, t]);
      ...
      if (!open) {
        return h("button", {
          type: "button",
          className: "dsh-git-tree-trigger",
      ...
      const svgW = nodeX(Math.max(maxCol, 0)) + 280;
          ...
          h("svg", { className: "dsh-git-tree-svg", width: svgW, height: svgH, role: "img" },
            ...
            h("g", null,
              graph.rows.map((row, i) => {
                ...
                const refText = c.refs.map((r) => r.replace(/^tag: /, "")).join(" ");
                const label = `${refText}${refText ? " " : ""}${c.subject}`;
    ...
    //#region css
    const CSS = [
      ".dsh-git-tree-trigger{display:flex;align-items:center;gap:6px;padding:5px 10px;border-radius:6px;border:1px solid transparent;background:transparent;color:var(--dsw-alias-label-secondary,#8b949e);cursor:pointer;font-size:12px;font-family:inherit;white-space:nowrap}",
      ".dsh-git-tree-trigger:hover{background:var(--dsw-alias-bg-overlay,#161b22);color:var(--dsw-alias-label-primary,#e6edf3)}",
      ".dsh-git-tree-overlay{position:fixed;inset:0;z-index:1200;display:flex;align-items:center;justify-content:center;background:color-mix(in srgb,var(--dsw-alias-bg-base,#0d1117) 70%,transparent)}",
      ".dsh-git-tree-panel{display:flex;flex-direction:column;width:min(940px,92vw);height:min(660px,88vh);background:var(--dsw-alias-bg-base,#0d1117);color:var(--dsw-alias-label-primary,#e6edf3);border:1px solid var(--dsw-alias-border-strong,#30363d);border-radius:10px;box-shadow:0 16px 60px rgb(0 0 0/.5);overflow:hidden}",
      ".dsh-git-tree-header{display:flex;align-items:center;gap:10px;padding:10px 14px;border-bottom:1px solid var(--dsw-alias-border-strong,#30363d)}",
      ".dsh-git-tree-title{font-size:14px;font-weight:600;margin-right:auto}",
      ".dsh-git-tree-workspace{max-width:280px;padding:4px 8px;border-radius:6px;border:1px solid var(--dsw-alias-border-strong,#30363d);background:var(--dsw-alias-bg-overlay,#161b22);color:inherit;font-size:12px}",
      ".dsh-git-tree-refresh,.dsh-git-tree-close,.dsh-git-tree-retry{padding:4px 10px;border-radius:6px;border:1px solid var(--dsw-alias-border-strong,#30363d);background:transparent;color:inherit;cursor:pointer;font-size:12px;line-height:1.2}",
      ".dsh-git-tree-refresh:hover,.dsh-git-tree-close:hover,.dsh-git-tree-retry:hover{background:var(--dsw-alias-bg-overlay,#161b22)}",
      ".dsh-git-tree-body{flex:1;overflow:auto;min-height:0}",
      ".dsh-git-tree-scroll{overflow:auto;padding:6px 14px 8px}",
      ".dsh-git-tree-svg{display:block}",
      ".dsh-git-tree-edge{stroke:var(--dsw-alias-label-secondary,#8b949e);stroke-width:1.5;opacity:.55;stroke-linecap:round}",
      ".dsh-git-tree-node{cursor:pointer}",
      ".dsh-git-tree-label{font-size:12px;fill:var(--dsw-alias-label-primary,#e6edf3);font-family:ui-monospace,SFMono-Regular,Menlo,monospace;dominant-baseline:middle;user-select:none}",
      ".dsh-git-tree-message{padding:28px 20px;text-align:center;color:var(--dsw-alias-label-secondary,#8b949e);font-size:13px;display:flex;flex-direction:column;gap:12px;align-items:center}",
      ".dsh-git-tree-error{color:var(--dsw-alias-state-error-primary,#f85149)}",
      ".dsh-git-tree-footer{display:flex;align-items:center;gap:14px;padding:8px 14px;border-top:1px solid var(--dsw-alias-border-strong,#30363d);font-size:12px;color:var(--dsw-alias-label-secondary,#8b949e)}",
      ".dsh-git-tree-search{flex:1;min-width:120px;padding:4px 8px;border-radius:6px;border:1px solid var(--dsw-alias-border-strong,#30363d);background:var(--dsw-alias-bg-overlay,#161b22);color:inherit;font-size:12px}",
      ".dsh-git-tree-meta{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}",
      ".dsh-git-tree-notice{color:var(--dsw-alias-state-success-primary,#3fb950);white-space:nowrap}"
    ].join("");
    //#endregion
