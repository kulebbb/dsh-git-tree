window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function GitTreePanel({ wide, t, useSessions, useWorkspaces, fetchGraph: loadGraph }) {
      ...
      const svgW = Math.ceil(nodeX(maxCol) + LABEL_OFFSET + Math.max(labelWidth, LABEL_MIN_WIDTH) + LABEL_PAD_RIGHT);
      ...
      if (loading && !payload) {
        ...
        bodyNode = h("div", { className: "dsh-git-tree-scroll" },
          h("svg", { className: "dsh-git-tree-svg", width: svgW, height: svgH, role: "img" },
            h("g", null,
              buildEdgePaths(graph).map((seg, i) =>
                h("path", {
                  key: `edge-${i}`,
                  d: seg.d,
                  fill: "none",
            ...
            h("g", null,
              graph.rows.map((row, i) => {
                ...
                return h("g", {
                  ...
                  h("text", { x: nodeX(row.col) + LABEL_OFFSET, y: 0, className: "dsh-git-tree-label" }, label),
                  h("text", { x: nodeX(row.col) + LABEL_OFFSET, y: DATE_LINE_DY, className: "dsh-git-tree-date" }, formatLocalTime(c.date))
                );
              })
            )
          )
    ...
    const CSS = [
      ...
      `.dsh-git-tree-date{font-size:${DATE_FONT_SIZE}px;font-family:var(--ds-font-family-code);fill:var(--dsw-alias-label-tertiary);dominant-baseline:middle;user-select:none}`,
