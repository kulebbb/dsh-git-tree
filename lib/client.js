  ...
  factory: (require) => {
    ...
    function measureRowWidths(rows) {
      ...
      try {
        ...
        if (!measureCtx) return rows.map(() => ({ subject: 0, full: 0 }));
        const family = getComputedStyle(document.documentElement)
          .getPropertyValue("--ds-font-family-code").trim() || "monospace";
        measureCtx.font = `${LABEL_FONT_SIZE}px ${family}`;
        return rows.map((r) => {
          const subject = measureCtx.measureText(labelOf(r.commit)).width;
          const full = measureCtx.measureText(rowTextOf(r.commit)).width;
          return { subject, full };
        });
      } catch {
        return rows.map(() => ({ subject: 0, full: 0 }));
      }
    }
    //#endregion
    ...
    function GitTreePanel({ wide, t, useSessions, useWorkspaces, fetchGraph: loadGraph }) {
      ...
      // row to dodge edge lines), floored by a comfortable minimum width.
            ...
            h("g", null,
              graph.rows.map((row, i) => {
                ...
                return h("g", {
                  ...
                  h("text", { x: labelXs[i], y: 0, className: "dsh-git-tree-label" }, subject),
                  h("text", { x: labelXs[i] + w.subject, y: 0, className: "dsh-git-tree-hash" }, hashSuffix),
